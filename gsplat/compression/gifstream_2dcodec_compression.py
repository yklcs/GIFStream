...
@dataclass
class GIFStream2dcodecCompression:
    ...
    def _get_compress_fn(self, param_name: str) -> Callable:
        compress_fn_map = {
            ...
            "offsets": _compress_png_kbit,
            "anchor_features": _compress_png_kbit,
        }
    ...
    def _get_decompress_fn(self, param_name: str) -> Callable:
        decompress_fn_map = {
            ...
            "offsets": _decompress_png_kbit,
            "anchor_features": _decompress_png_kbit,
        }
...
def _compress_gaussian(
    compress_dir: str, param_name: str, params: Tensor, channel_len: int, means: Tensor, scales: Tensor, q_scale: float, entropy_coder, gaussian_encoder, **kwargs
) -> Dict[str, Any]:
    """compress parameters using gaussian distribution.

    Args:
        compress_dir (str): compression directory
        param_name (str): parameter field name
        params (Tensor): params to compress
        channel_len (int): progressive compress channel number
        means (Tensor): predicted params means
        scales (Tensor): predicted params scales
        q_scale (Tensor): to change quantization step
        entropy_coder, gaussian_encoder : encode params to bin
    ...
    params_norm = torch.round(((params - mins) / (maxs - mins + 1e-6) - 0.5) * (2 ** 8 - 1) / q_scale).view((params.shape[0],-1,channel_len))

    means = (means / q_scale).view((params.shape[0],-1,channel_len))
    scales = (scales / q_scale).view((params.shape[0],-1,channel_len))

    bits = 0
    for i in range(params_norm.shape[1]):
        output_path = os.path.join(compress_dir,f"{param_name}_{i}.bin")
        entropy_coder.reset_encoder()
        _ = gaussian_encoder.encode(params_norm[:,i,:] - means[:,i,:], scales[:,i,:])
        bit_stream = entropy_coder.flush_encoder()
    ...
    meta = {
        ...
        "maxs": maxs.tolist(),
        "q_scale": q_scale,
        "steps": params_norm.shape[1]
    }
    return meta

def _decompress_gaussian(compress_dir: str, param_name: str, meta: Dict[str, Any], entropy_model,entropy_coder, gaussian_encoder, prefix=None) -> Tensor:
    """Decompress parameters from PNG file.
    ...
    Returns:
        Tensor: parameters
    """
    for ind in range(meta["steps"]):
        output_path = os.path.join(compress_dir, f"{param_name}_{ind}.bin")
        if ind == 0 and prefix is None:
            prefix = torch.zeros([meta["shape"][0],meta["channel_len"]])
            scales = entropy_model(prefix)
        bit_stream = decode_x(output_path)
        entropy_coder.set_stream(bit_stream)
        params = gaussian_encoder.decode_stream(scales).cuda() if ind == 0 else torch.cat([params,gaussian_encoder.decode_stream(scales).cuda()],dim=1)
        prefix = params
    return params
