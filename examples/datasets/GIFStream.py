...
class Dataset:
    ...
    def __init__(
        ...
        indices = np.arange(len(self.parser.image_names)*GOP_size)
        self.cameras_length = len(self.parser.image_names)
        if split == "train":
    ...
    def __getitem__(self, item: int) -> Dict[str, Any]:
        ...
        camera_path = self.parser.image_paths[cam]
        image_path = os.path.join(os.path.dirname(os.path.dirname(camera_path)), f"cam{cam:02d}", f"{(frame_idx+1):03d}." + os.path.basename(camera_path).split(".")[-1])
        ...
        data = {
            ...
            "camera_id": camera_id if self.split == "train" else -1,
        }
        if mask is not None:
