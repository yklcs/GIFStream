...
class CameraEmbedding(torch.nn.Module):
    """Appearance optimization module."""
    ...
    def forward(
        ...
        # Camera embeddings
        if embed_ids < 0:
            embeds = torch.zeros(C, self.embed_dim, device=embed_ids.device)
        else:
            embeds = self.embeds(embed_ids)  # [C, D2]
        return embeds
...
def find_k_neighbors(input, k):
    """
    Find k nearest neighbors for each point in a point cloud. During training, randomly shuffle the order of points to simulate the effect of reordering during encoding.
    ...
    points = input[shuffled_indices].detach().cpu().numpy()
    q_points = input.detach().cpu().numpy()
    
    tree = KDTree(points)
    
    distances, indices = tree.query(q_points, k=k+1)
    indices = shuffled_indices[indices.squeeze().flatten()].reshape(indices.shape)
    
    return torch.tensor(distances[:, 1:],device=input.device,dtype=input.dtype), indices[:, 1:]
...
def get_gifstream_sorted_loss(splats: Dict[str, Tensor], kernel_size = 5, sigma=0.5) -> Tensor:
    ...
    params_to_sort = torch.cat([params_to_sort, torch.zeros([padded_length,params_to_sort.shape[-1]])], dim=0)
    shuffled_indices = torch.randperm(
    ...
    sorted_params, _ = sort_with_plas(
        grid.permute(2, 0, 1), improvement_break=1e-4, verbose=False
    )
    blurred = kornia.filters.gaussian_blur2d(
            sorted_params.detach(),
            kernel_size=(kernel_size, kernel_size),
            sigma=(sigma,sigma),
            border_type="circular",
    )
    loss = torch.nn.functional.mse_loss(blurred, sorted_params)
    return loss
